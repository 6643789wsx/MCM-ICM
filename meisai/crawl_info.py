...
def genHeader():
    ...
    return headers
index=['Est. Forestay Len.:','S.A./Disp. (calc.):','S.A. Total (100% Fore + Main Triangles)','S.A. Main:','S.A. Fore:','ISP:','SPL/TPS:','E:','P:','J:','I:','S#:','Capsize Screening Formula:','Comfort Ratio:','Disp./Len.:','Bal./Disp.:','Designer:','# Built: ','First Built:','Ballast Type:','Construction:','Disp./Len.:','Bal./Disp.:','Ballast:','Beam:','LWL:','LOA:','Hull Type:','S.A.(reported):','Draft(max):','Displacement:','S.A./Disp.','KSP','Fuel','Mast Height from DWL','Water','Rigging Type:','Make','Type']
...
if __name__ == "__main__":
    ...
    df.to_csv("finish_single.csv")
    df = pandas.DataFrame(catamarans).T
    df.to_csv("finish_Catamarans.csv")